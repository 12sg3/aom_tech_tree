...
data_fields_unit_dict = units_dict_GD[units_dict_GD.keys()[0]]

for unit_key, data_field in zip(units_dict_GD.keys(), data_fields_unit_dict):
    display_name = units_dict_GD[unit_key]['Display_Name']
    if (display_name in caret_duplicates_dictXX_keys):
        for version in caret_duplicates_dictXX[display_name]["versions"]:
            generate_new_dict_item_from_game_data(unit_key, data_fields_unit_dict, version = version)
...
# print('units_dict_GD: ', units_dict_GD)
# print('buildings_dict_GD: ', buildings_dict_GD)