...
dir_path_god_powers = 'images/text_pics/old_desc_cost_only/basic-desc/god-powers/'
images_to_text_entries_god_powers = os.listdir(dir_path_god_powers)

# print(f'images_to_text_entries: {images_to_text_entries}')

new_df_entries = []

for img in images_to_text_entries_god_powers:
    ...
    basic_description = get_description_from_img_tech(img_file_name, f'{dir_path_god_powers}{img_file_name.replace('_', ' ')}')
    ...
    if img_name in names:
        ...
        data_df.loc[data_df[NAME] == img_name, TYPE] = GOD_POWER

    else:
        new_df_entries.append(pandas.DataFrame([{NAME: img_name, DESCRIPTION: basic_description, TYPE: GOD_POWER}]))
